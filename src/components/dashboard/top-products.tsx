...
export function TopProducts({ products }: TopProductsProps) {
  // Divide los productos en 2 columnas (o 3 si hay muchos)
  const columns = 2;
  const chunked = Array.from({ length: columns }, (_, col) =>
    products.filter((_, idx) => idx % columns === col)
  );

  return (
    <Card className="shadow-lg rounded-xl w-full flex flex-col">
      <CardHeader>
        <CardTitle className="text-xl font-semibold text-foreground">Top Highest Grossing Products</CardTitle>
      </CardHeader>
      <CardContent className="flex-grow flex flex-col overflow-hidden p-0 min-h-0">
        <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 p-4`}>
          {chunked.map((productsCol, colIdx) => (
            <ul key={colIdx} className="space-y-1">
              {productsCol.map((product, index) => {
                ...
                return (
                  <li key={product.id} className="rounded-md hover:bg-muted/60 transition-colors duration-150">
                    ...
                    {index < productsCol.length - 1 && <Separator className="my-0 mx-2 border-border/60" />}
                  </li>
                );
              })}
            </ul>
